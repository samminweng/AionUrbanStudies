...
class BERTModelDocCluster:
    ...
    def re_cluster_outliers_by_hdbscan(self):
        try:
            ...
            cluster_df.rename(columns={'HDBSCAN_Cluster': 'Cluster'}, inplace=True)
            # Assignment doc vectors
            cluster_df['DocVectors'] = self.text_df['DocVectors'].tolist()
            ...
            for dimension in [768, 200, 150, 100, 95, 90, 85, 80, 75, 70, 65, 60,
                        55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 9, 8, 7, 6, 5]:
                # Apply UMAP to reduce the dimensions of document vectors
                if dimension < 768:
                    # Run HDBSCAN on reduced dimensional vectors
                    ...
                    # Run HDBSCAN on raw vectors
                    reduced_vectors = np.vstack(doc_vectors)  # Convert to 2D numpy array
                # Cluster the (reduced) document vectors with different parameters
                results = BERTModelDocClusterUtility.cluster_outliers_experiments_by_hdbscan(dimension, reduced_vectors)
                # Output the clustering results of a dimension
