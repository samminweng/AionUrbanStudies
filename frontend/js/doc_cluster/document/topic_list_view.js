...
function TopicListView(cluster_no, topic_words, documents){
    ...
    function _createUI(){
        ...
        const table = $('<table class="table table-sm"><thead><tr>' +
            '<th>Top ' + topic_words.length + ' Topic</th>' +
            '<th>Score</th>' +
            '<th># Articles</th>' +
        ...
        for(const topic_word of topic_words){
            const topic = topic_word['collocation'];
            const score = topic_word['score']
            ...
            const score_div = $('<td>' + parseFloat(score.toFixed(1)) + '</td>');
            const num_doc_div = $('<td>' + topic_docs.length + '</td>');
            row.append(topic_div);
            row.append(score_div);
            row.append(num_doc_div);
            tbody.append(row);
        }
