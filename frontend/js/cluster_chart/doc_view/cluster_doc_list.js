function ClusterDocList(cluster_no, corpus_data, cluster_data) {
    ...
    const cluster_link = $('<a target="_blank" href="term_sunburst.html?cluster='+ cluster_no + '">Article Group #' + (cluster_no + 1) + '</a>');
    ...
    function create_cluster_terms_key_phrase_topics(){
        ...
        $('#cluster_terms').empty();
        const container = $('<div><span class="fw-bold">Distinct Terms: </span></div>');
        const term_p = $('<p></p>');
